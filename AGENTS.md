...
The application runs as a single Docker container (or Node.js process):
- **opencoach** - SvelteKit web app with node-cron for background jobs

### 3.1 The "Loops" (Cron Jobs via node-cron)

1. **Token Refresh (Every 30 min):** Refreshes Garmin session tokens.
...
* **Authentication:** Direct login via garmin-connect npm package (requires 2FA disabled).
...
### 5.2 Service: The AI Coach (`src/lib/server/coach.ts`)
...
* **Step 1:** Garmin authentication via login form.
