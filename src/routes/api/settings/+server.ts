...
import { validateCredentials } from '$lib/server/garmin';
...
export const POST: RequestHandler = async ({ request, platform }) => {
	...
	// Validate Garmin credentials if provided (skip if requested)
	if (payload.garmin_email && payload.garmin_password) {
		if (payload.skip_validation) {
			...
			const isValid = await validateCredentials(payload.garmin_email, payload.garmin_password);
			if (!isValid) {
				validationErrors.push('Invalid Garmin credentials (try "Skip Validation" if Garmin is blocking automated login)');
			} else {
				settingsToSave[SETTING_KEYS.GARMIN_EMAIL] = payload.garmin_email;
				settingsToSave[SETTING_KEYS.GARMIN_PASSWORD] = payload.garmin_password;
			}
