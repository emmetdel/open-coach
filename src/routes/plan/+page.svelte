<script lang="ts">
    ...
    import {Calendar, RefreshCw, Settings, Zap, Activity, TrendingUp} from "lucide-svelte";
...
<div class="min-h-screen bg-gradient-to-br from-slate-925 via-slate-900 to-slate-925">
    ...
    <div class="relative z-10">
        <header class="border-b border-slate-800/50 bg-slate-925/90 backdrop-blur-xl sticky top-0 z-50">
            <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
                ...
                <nav class="hidden md:flex items-center gap-1">
                    ...
                    <a href="/plan">
                        <Button variant="ghost" size="sm" class="h-9 px-4 bg-slate-800">
                ...
                <div class="flex items-center gap-1 sm:gap-2">
                    <Button onclick={regeneratePlan} variant="outline" size="sm" disabled={regenerating} class="hidden sm:flex h-9 px-3">
                        <RefreshCw class={`h-4 w-4 sm:mr-2 ${regenerating ? "animate-spin" : ""}`}/>
                        <span class="hidden sm:inline">{regenerating ? "Regenerating..." : "Regenerate"}</span>
                    </Button>
                    <a href="/settings">
                        <Button variant="ghost" size="sm" title="Settings" class="h-9 w-9 p-0">
            ...
            <div class="md:hidden border-t border-slate-800/50">
                <nav class="mx-auto max-w-7xl px-4 py-2 flex items-center justify-around">
                    <a href="/" class="flex flex-col items-center gap-1 py-2 px-3 text-slate-400 hover:text-white transition-colors">
                        <Activity class="h-5 w-5"/>
                        <span class="text-xs">Home</span>
                    </a>
                    <a href="/analytics" class="flex flex-col items-center gap-1 py-2 px-3 text-slate-400 hover:text-white transition-colors">
                        <TrendingUp class="h-5 w-5"/>
                        <span class="text-xs">Analytics</span>
                    </a>
                    <a href="/plan" class="flex flex-col items-center gap-1 py-2 px-3 text-white">
                        <Calendar class="h-5 w-5"/>
                        <span class="text-xs">Plan</span>
                    </a>
                    <button onclick={regeneratePlan} class="flex flex-col items-center gap-1 py-2 px-3 text-slate-400 hover:text-white transition-colors" disabled={regenerating}>
                        <RefreshCw class={`h-5 w-5 ${regenerating ? "animate-spin" : ""}`}/>
        ...
        <main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
            {#if message}
                <div class="mb-6 rounded-xl bg-forest-500/10 px-4 py-3 text-sm text-forest-400">{message}</div>
            {/if}
            ...
            {:else}
                <!-- Plan Overview -->
                <div class="mb-8 rounded-xl border border-forest-500/30 bg-gradient-to-br from-slate-850 to-slate-900 p-6">
                    <h2 class="font-display text-2xl font-bold text-white">{data.planName}</h2>
                    <p class="mt-2 text-sm text-slate-400">
                        Week <span class="font-bold text-white">{data.currentWeek}</span> of <span class="font-bold text-white">{data.totalWeeks}</span>
                    </p>
                    <div class="mt-4 flex gap-1">
                        {#each Array(data.totalWeeks) as _, i}
                            <div class="h-2 flex-1 rounded-full {i < data.completedWeeks ? 'bg-forest-500' : i === data.currentWeek - 1 ? 'bg-forest-500/50' : 'bg-slate-700'}"></div>
                        {/each}
                    </div>
                </div>

                <!-- Interactive Calendar -->
                <TrainingCalendar runs={calendarRuns} onMove={handleMove} onStatusChange={handleStatusChange}/>
            {/if}
        </main>
    </div>
</div>
